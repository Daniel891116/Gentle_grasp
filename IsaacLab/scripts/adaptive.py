...
parser.add_argument("--out_file", type=str, default="adaptive.pt", help="Path to save the data")
...
gripper_stiffness = 150 * torch.ones(num_envs, 1).to(device)
gripper_damping = 15 * torch.ones(num_envs, 1).to(device)
...
for t in range(iterations):
    ...
    # adaptively change the stiffness and damping if slipping
    if slipping.any():
        gripper_stiffness[slipping] *= 1.2
        gripper_damping[slipping] *= 1.2
        env_ids = torch.nonzero(slipping, as_tuple=False).squeeze(-1)
        robot.write_joint_stiffness_to_sim(gripper_stiffness[slipping], joint_ids=finger_ids, env_ids=env_ids)
        robot.write_joint_damping_to_sim(gripper_damping[slipping], joint_ids=finger_ids, env_ids=env_ids)
        print(f"slipping, adapted stiffness and damping")
        print(f"gripper stiffness: {gripper_stiffness[slipping]}")
        print(f"gripper damping: {gripper_damping[slipping]}")
